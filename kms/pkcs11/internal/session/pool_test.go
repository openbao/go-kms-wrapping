...
func Test(t *testing.T) {
	...
	t.Run("Login+Drop", func(t *testing.T) {
		t.Run("Cache", func(t *testing.T) {
			...
			p1, err := Login(ctx, mod, token1, pin2)
			...
			p1, err = Login(ctx, mod, token1, pin1)
			...
			p2, err := Login(ctx, mod, token2, pin2)
			require.NoError(t, err, "correct pin should work")
		...
		t.Run("Sharing", func(t *testing.T) {
			...
			p2, err := Login(ctx, mod, token1, pin2)
			...
			p2, err = Login(ctx, mod, token1, pin1)
			require.NoError(t, err, "correct pin should work")
			require.NotNil(t, p2, "should return a pool")
			...
			synctest.Test(t, func(t *testing.T) {
				...
				defer cancel()
				_, err := Login(timeoutCtx, mod, token1, pin1)
				require.Error(t, err, "should time out waiting for pool to go live")

				errs := make([]error, 10)
				for i := range errs {
					go func(i int) {
						_, errs[i] = Login(t.Context(), mod, token1, pin1)
					}(i)
				}
			...
			synctest.Test(t, func(t *testing.T) {
				...
				for i := range len(errs) / 2 {
					go func(i int) {
						refs[i], errs[i] = Login(t.Context(), mod, token1, pin1)
					}(i)
				}
	...
	t.Run("Get+Close", func(t *testing.T) {
		...
		t.Run("Drop", func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				p, err := Login(ctx, mod, token1, pin1)
				require.NoError(t, err, "correct pin should work")
