	...
	if opts.WithConfigMap != nil {
		for k, v := range opts.WithConfigMap {
			...
			case "read_timeout":
				...
				opts.withReadTimeout = uint8(timeout)
			case "write_timeout":
				var err error
				var timeout uint64
				timeout, err = strconv.ParseUint(v, 10, 64)
				...
				opts.withWriteTimeout = uint8(timeout)
			}
		}
	}
...
type options struct {
	*wrapping.Options
	...
	withReadTimeout  uint8
	withWriteTimeout uint8
}
...
// WithReadTimeout provides a way to chose the read timeout
func WithReadTimeout(with uint8) wrapping.Option {
	return func() interface{} {
		return OptionFunc(func(o *options) error {
			o.withReadTimeout = with
			return nil
		})
	}
}

// WithWriteTimeout provides a way to chose the write timeout
func WithWriteTimeout(with uint8) wrapping.Option {
	return func() interface{} {
		return OptionFunc(func(o *options) error {
			o.withWriteTimeout = with
			return nil
		})
	}
}
